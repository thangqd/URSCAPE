...
# -*- coding: utf-8 -*-
# --------------------------------------------------------
#    urscapegis_dialogs - Dialog classes for urscape
#
#    begin                : 01/02/2018
...
# ------------------------------------------------------------------------------
#    urscape_dialog - base class for urscape dialogs containing utility functions
# ------------------------------------------------------------------------------
class urscape_dialog(QtWidgets.QDialog):
    def __init__(self, iface):
        QtWidgets.QDialog.__init__(self)
        self.iface = iface

    def urscape_initialize_spatial_output_file_widget(self, file_widget, name, ext = ".shp"):
        initial_file_name = self.urscape_temp_file_name(name, ext)
    ...
    def urscape_temp_file_name(self, name, ext):
    ...
    def urscape_fill_list_widget_with_vector_layers(self, list_widget):
        # Add layers not in the list
        #or layer in self.iface.mapCanvas().layers():
    ...
    def urscape_find_layer(self, layer_name):
        if not layer_name:
            return None
    ...
    def urscape_set_status_bar(self, status_bar, status_lable):
    ...
    def urscape_status_callback(self, percent_complete, lable):
        try:           
...
#    urscape Create Grid
# --------------------------------------------------------
class urscape_creategrid_dialog(urscape_dialog, Ui_urscape_creategrid_form):		
    def __init__(self, iface):
        urscape_dialog.__init__(self, iface)		
        self.setupUi(self)	
        self.CboInput.setFilters(QgsMapLayerProxyModel.PolygonLayer)
        self.urscape_fill_list_widget_with_vector_layers(self.lsLayers)	
        ...
        self.urscape_initialize_spatial_output_file_widget(self.output_file_name,'grid')
        
                
    def run(self):             	
        self.LblStatus.clear()	
        gridtype= self.CboGridType.currentIndex()
        layer = self.CboInput.currentLayer()
        extend = self.CboInput.currentLayer().extent()
        if layer is None:
            return u'No selected layers!'
        cellsize = self.spinCellsize.value()
        gridCRS = self.CboInput.currentLayer().crs()      
        output = str(self.output_file_name.filePath())

        excluded_layers = []
        for item in self.lsLayers.selectedItems():
            excluded_layer = self.urscape_find_layer(item.text())            
            excluded_layers.append(excluded_layer)
        if len(excluded_layers)==0:  # 0 items
            message = urscape_creategrid(gridtype,layer,cellsize, gridCRS,output,self.urscape_status_callback)
            if message != None:
                QMessageBox.critical(self.iface.mainWindow(), "Create Grid", message)	
            else: 
                self.LblStatus.setText('Completed')       
        else : #> 0 items
            message = urscape_creategrid2(gridtype,layer,cellsize, gridCRS,excluded_layers,output,self.urscape_status_callback)
            if message != None:
                QMessageBox.critical(self.iface.mainWindow(), "Create Grid", message)	
            else: 
                self.LblStatus.setText('Completed')   
...
class urscape_importer_dialog(urscape_dialog, Ui_urscape_importer_form):		
    def __init__(self, iface):
        urscape_dialog.__init__(self, iface)		
        self.setupUi(self)      
